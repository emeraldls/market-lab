...
pub fn expand_args(args: impl IntoIterator<Item = OsString>) -> Result<Vec<OsString>> {
    ...
    let command = script_command(&args);
    if command.is_none() && explicit_config.is_none() {
        return Ok(args);
    }
    let Some((script_idx, mode_idx, mode)) = command else {
        bail!("--config currently supports only `script run` and `script backtest`");
    };
    ...
    append_config_flags(&mut expanded, &config, mode)?;
...
fn script_command(args: &[OsString]) -> Option<(usize, usize, &str)> {
    let script_idx = args.iter().position(|arg| arg == "script")?;
    let mode_idx = script_idx + 1;
    let mode = args.get(mode_idx)?.to_str()?;
    matches!(mode, "run" | "backtest").then_some((script_idx, mode_idx, mode))
}
